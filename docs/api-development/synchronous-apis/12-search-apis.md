...
<Standard id="MSDAS_MUST_NOT_PUT_SENSITIVE_DATA_IN_QUERY_PARAMS" type="MUST NOT">
...
<Standard id="MSDAS_MUST_USE_PARAMETERISED_QUERIES" type="MUST">
Data access code MUST use parameterised queries or an equivalent safe data-access mechanism
</Standard>

This applies to any user-supplied input reaching a data store (e.g. parameterised `SqlCommand`/Entity Framework in .NET, prepared statements in JDBC, parameterised ORM queries). Concatenating user input directly into a query string, in any form, <Standard inline id="MSDAS_MUST_NOT_CONCATENATE_USER_INPUT_INTO_QUERIES" type="MUST NOT" toolTip="Concatenating user input directly into a query string, in any form, must not be used; use parameterised queries or an equivalent safe data-access mechanism.">MUST NOT</Standard> be used.

<Standard id="MSDAS_MUST_ENCODE_RENDERED_OUTPUT" type="MUST">
