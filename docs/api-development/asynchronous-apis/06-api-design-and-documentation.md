...
<Standard id="MSDAS_MUST_DOCUMENT_ASYNCHRONOUS_APIS" type="MUST">
...
<Standard id="MSDAS_SHOULD_USE_CLOUDEVENTS_SPECIFICATION" type="RECOMMENDED">
...
Using a consistent, well-documented event schema allows API Consumers and Publishers to design against a known message structure, accelerating integration on both sides. API Providers <Standard inline id="MSDAS_SHOULD_VALIDATE_OUTGOING_MESSAGES" type="SHOULD" toolTip="API Providers should validate outgoing messages against their published JSON Schema before publishing them.">SHOULD</Standard> validate outgoing messages against their published JSON Schema before publishing them.

<Standard id="MSDAS_MAY_OFFER_EVENT_CATALOGUE" type="MAY">
